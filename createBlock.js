...
const fileSources = {
	pug: `mixin {blockName}()\n\t.{blockName}\n\t\t| {blockName}\n`,
	scss: `// Here goes BEM-block styles  .{blockName} \n// and it elements, modifier, pseudoselectors, @media-queries... \n\n.{blockName} \{\n\t\n\}`,
	js: `//(function() {})();`
};
...
function validateBlockName(blockName) {
	return new Promise((resolve, reject) => {
		...
		if (isValid) {
			...
			const errMsg = (
				`ERR>>> Wrong block name '${blockName}'\n` +
				`ERR>>> Block name must containe letters, numbers, minus sign.`
			);
			reject(errMsg);
		}
	});
